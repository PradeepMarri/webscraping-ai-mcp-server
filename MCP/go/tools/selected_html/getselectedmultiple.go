...
func GetselectedmultipleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["selectors"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("selectors=%v", val))
		}
		...
		url := fmt.Sprintf("%s/selected-multiple%s", cfg.BaseURL, queryString)
...
func CreateGetselectedmultipleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_selected-multiple",
		mcp.WithDescription("HTML of multiple page areas by URL and CSS selectors"),
		mcp.WithArray("selectors", mcp.Description("Multiple CSS selectors (null by default, returns whole page HTML)")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetselectedmultipleHandler(cfg),
	}
}
