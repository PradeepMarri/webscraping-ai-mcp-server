...
func GettextHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["text_format"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("text_format=%v", val))
		}
		if val, ok := args["return_links"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("return_links=%v", val))
		}
		...
		url := fmt.Sprintf("%s/text%s", cfg.BaseURL, queryString)
...
func CreateGettextTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_text",
		mcp.WithDescription("Page text by URL"),
		mcp.WithString("text_format", mcp.Description("Format of the text response (plain by default). \"plain\" will return only the page body text. \"json\" and \"xml\" will return a json/xml with \"title\", \"description\" and \"content\" keys.")),
		mcp.WithBoolean("return_links", mcp.Description("[Works only with text_format=json] Return links from the page body text (false by default). Useful for building web crawlers.")),
